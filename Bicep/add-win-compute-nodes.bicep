...
var headNodeFullname = split(trim(headNodeList), ',')[0]
...
module computeNodes 'shared/compute-node.bicep' = [
  for i in range(0, computeNodeNumber): if (!useVmssForCN) {
    ...
    params: {
      subnetId: subnetRef
      vmName: '${_computeNodeNamePrefix}${padLeft(string((i + computeNodeNameStartIndex)), 3, '0')}'  //TODO: make a cunstom function for the name
      ...
      headNodeList: trim(headNodeList)
      joinDomain: (!empty(domainName))
